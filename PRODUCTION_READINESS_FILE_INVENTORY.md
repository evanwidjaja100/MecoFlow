...
PASS apps/api/src/logger.ts — pino redact auth/cookie/password/token/secret + *. , error only classification/type
PASS apps/api/src/main.ts — graceful shutdown 10s
PASS apps/api/src/monitoring/* — private Prometheus text, bounded labels
...
PASS infra/production/*.example.json — contract correct, NA (examples)
PASS infra/scripts/* — provisioning correct
